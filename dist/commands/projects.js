...
export function setupProjectsCommands(program) {
    ...
    projects
        ...
        const teamNames = options.teams
            ...
            .map((t) => t.trim())
            .filter(Boolean);
        const teamIds = await Promise.all(teamNames.map((t) => resolveTeamId(ctx.sdk, t)));
        const input = {
            name,
            teamIds,
        };
        ...
        if (options.members) {
            const memberNames = options.members
                ...
                .map((m) => m.trim())
                .filter(Boolean);
            input.memberIds = await Promise.all(memberNames.map((m) => resolveUserId(ctx.sdk, m)));
        }
        if (options.priority) {
            input.priority = parsePriority(options.priority);
        }
        if (options.status) {
            input.statusId = await resolveProjectStatusId(ctx.gql, options.status);
        }
    ...
    projects
        ...
        .option("--labels <labels>", "comma-separated label names or UUIDs")
        .action(handleCommand(async (...args) => {
        const [project, options, command] = args;
        const ctx = createContext(command.parent.parent.opts());
        const projectId = await resolveProjectId(ctx.sdk, project);
        const input = {};
        if (options.name) {
        ...
        if (options.teams) {
            const teamNames = options.teams
                ...
                .map((t) => t.trim())
                .filter(Boolean);
            input.teamIds = await Promise.all(teamNames.map((t) => resolveTeamId(ctx.sdk, t)));
        }
        if (options.labels) {
            const labelNames = options.labels
                .split(",")
        ...
        if (Object.keys(input).length === 0) {
            throw invalidParameterError("update options", "at least one option must be provided");
        }
